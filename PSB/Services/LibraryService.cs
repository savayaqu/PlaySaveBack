...
namespace PSB.Services
{
    public class LibraryService
    {
        ...
        private readonly TextBlock _gamesCountTextBlock; // Для отображения количества игр
        private readonly NavigationViewItem _libraryHeader; // Заголовок "БИБЛИОТЕКА"

        // Словарь для хранения состояния сворачивания категорий
        private readonly Dictionary<string, bool> _categoryCollapseState = [];
        ...
        private void OnSearchTextChanged(AutoSuggestBox sender, AutoSuggestBoxTextChangedEventArgs args)
        {
            if (args.Reason == AutoSuggestionBoxTextChangeReason.UserInput)
            {
                ...
                {
                    UpdateLibraryMenu([.. _profileViewModel.Libraries]);
                    return;
                }

                // Фильтруем игры по названию (включая сторонние игры)
                var filteredGames = _profileViewModel.Libraries
                    .Where(game => game.Game != null && game.Game.Name.Contains(searchText, System.StringComparison.CurrentCultureIgnoreCase) ||
                                   game.SideGame != null && game.SideGame.Name.Contains(searchText, System.StringComparison.CurrentCultureIgnoreCase))
                    .ToList();
        ...
        public void UpdateLibraryMenu(List<Library> filteredGames = null)
        {
            ...
            try
            {
                ...
                var gamesToDisplay = filteredGames ?? [.. _profileViewModel.Libraries];
                ...
                void AddCategory(string headerText, List<Library> games, bool isSideGame = false)
                {
                    ...
                    var header = new NavigationViewItem
                    {
                        ...
                        IsExpanded = !_categoryCollapseState.TryGetValue(headerText, out var isExpanded) || isExpanded,
                        SelectsOnInvoked = false // Отключаем выбор элемента
                    };
